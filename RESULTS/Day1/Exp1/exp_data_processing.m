...
addpath(fullfile('E:\Master Degree\Linux_file\Thesis\rosbags_new\Thruster_Dynamic'));

% Add the 'Ballast_Config_Input' path
addpath(fullfile('E:\Master Degree\Linux_file\Thesis\rosbags_new\Ballast_Config_Input'));

% Add the 'Util' path
addpath(fullfile('E:\Master Degree\Linux_file\Thesis\rosbags_new\Util'));
...
yLimits = ylim;
x_fill = [0, linspace(0, settling_time/dt, 100), settling_time/dt, settling_time/dt, 0]; % X coordinates for the filled area, closed loop
...
text(settling_time/(2*stop_time)*Row_Length, 1600, 'Stabilizing Phase', 'FontSize', 10, 'Color', 'black', 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle');
text((stop_time+settling_time)/(2*stop_time)*Row_Length, 1600, 'Maintain Phase', 'FontSize', 10, 'Color', 'black', 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle');
...
xlim([0, rel_alt_ts])
ylim([-1.25, 0.1])
xticks(0:rel_alt_ts/120*20:rel_alt_ts)
xticklabels(0:20:stop_time);

% Adding text annotations
text(settling_time/stop_time*rel_alt_ts/2, -1, 'Stabilizing Phase', 'FontSize', 10, 'Color', 'black', 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle');
text((stop_time/2 + settling_time/2)/stop_time*rel_alt_ts, -1, 'Maintain Phase', 'FontSize', 10, 'Color', 'black', 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle');
...
plot(data_roll.roll, 'LineWidth', 1.5, 'Color', color1);
hold on;
plot(phi_ref, 'LineWidth', 1.5, 'Color', color2);
hold on;
title('Roll angle displacement', 'FontSize', 12);
ylabel('radian', 'FontSize', 10);
set(gca, 'Color', [0.9 0.9 0.9]);
grid on
xlim([0, roll_ts])
ylim([-pi/4, pi/4])
xticks(0:roll_ts/120*20:roll_ts)
xticklabels(0:20:stop_time);

% Adding text annotations
text(settling_time/stop_time*roll_ts/2, -0.5, 'Stabilizing Phase', 'FontSize', 10, 'Color', 'black', 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle');
text((stop_time/2 + settling_time/2)/stop_time*roll_ts, -0.5, 'Maintain Phase', 'FontSize', 10, 'Color', 'black', 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle');
...
plot(data_pitch.pitch, 'LineWidth', 1.5, 'Color', color1);
hold on;
...
xlim([0, pitch_ts])
ylim([-pi/4, pi/4])
xticks(0:pitch_ts/120*20:pitch_ts)
xticklabels(0:20:stop_time);
...
lgd = legend('Recording', 'Reference', 'Orientation', 'horizontal', 'FontSize', 10, 'FontWeight', 'bold', 'FontName', 'Helvetica');
...
% %% PLOT FREE FLOAT
% figure('Name', 'Position and Orientation during Free Float Test', 'Position', [100, 100, 700, 900]);
% 
% % Define colors for the plots
% color1 = [0, 0.4470, 0.7410]; % blue
% color2 = [0.8500, 0.3250, 0.0980]; % red
...
% 
% % Create a single legend for the entire figure
% lgd = legend('Free Float', 'Orientation', 'horizontal', 'FontSize', 10, 'FontWeight', 'bold', 'FontName', 'Helvetica');
% lgd.Position = [0.4, 0.01, 0.2, 0.05];
% 
% % Adjust line thickness and color contrast for better visibility
% lineObjects = findall(gcf, 'Type', 'line');
