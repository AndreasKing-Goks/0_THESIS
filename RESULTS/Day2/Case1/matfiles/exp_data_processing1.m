...
% addpath(fullfile('/home/goks/thesis_ws/rosbags_new3/Thruster_Dynamic'));
...
Mean_Total_Thruster = (Mean_Total_Thruster * diag([1 1 1 -1 1 1])); % Reversing the heave force and pitch moment (up (x))sign
...
% Adding text annotations
text(settling_time/stop_time*rel_alt_ts/2, -1, 'Transient Phase', 'FontSize', 10, 'Color', 'black', 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle');
text((stop_time/2 + settling_time/2)/stop_time*rel_alt_ts, -1, 'Steady-State Phase', 'FontSize', 10, 'Color', 'black', 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle');
...
plot(data_roll.roll/pi*180, 'LineWidth', 1.5, 'Color', color1);
hold on;
...
lgd = legend('Experiment', 'Reference', 'Orientation', 'horizontal', 'FontSize', 10, 'FontWeight', 'bold', 'FontName', 'Helvetica');
...
rcrd_time = 90;
rls_time = 30;
rls_time_idx = rls_time/rcrd_time; 

start_idx_heave = floor(rls_time_idx*nb_rel_alt_ts);
start_idx_roll = floor(rls_time_idx*nb_roll_ts);
start_idx_pitch = floor(rls_time_idx*nb_pitch_ts);
...
% Plot Z
subplot(3,1,1);
plot(data_rel_alt_ff.rel_alt, 'LineWidth', 1.5, 'Color', color1);
hold on;
plot(data_rel_alt_nb.rel_alt(start_idx_heave:end), 'LineWidth', 1.5, 'Color', color2);
hold on;
title('Heave displacement', 'FontSize', 12);
ylabel('meter', 'FontSize', 10);
set(gca, 'Color', [0.9 0.9 0.9]);
grid on
xlim([0, ff_rel_alt_ts])
ylim([-1.25, 0.1])
xticks(0:ff_rel_alt_ts/rcrd_time*10:ff_rel_alt_ts)
xticklabels(0:10:rcrd_time); 
yline(0, 'k--');

% Plot Phi
subplot(3,1,2);
plot(data_roll_ff.roll/pi*180, 'LineWidth', 1.5, 'Color', color1);
hold on;
plot(data_roll_nb.roll(start_idx_roll:end)/pi*180, 'LineWidth', 1.5, 'Color', color2);
hold on;
title('Roll angle displacement', 'FontSize', 12);
ylabel('degree', 'FontSize', 10);
set(gca, 'Color', [0.9 0.9 0.9]);
...
xlim([0, ff_roll_ts])
ylim([-20, 20])
xticks(0:ff_roll_ts/rcrd_time*10:ff_roll_ts)
xticklabels(0:10:rcrd_time);
yline(0, 'k--');

% Plot Theta
subplot(3,1,3);
plot(-data_pitch_ff.pitch/pi*180, 'LineWidth', 1.5, 'Color', color1);
hold on;
plot(-data_pitch_nb.pitch(start_idx_pitch:end)/pi*180, 'LineWidth', 1.5, 'Color', color2);
hold on;
title('Pitch angle displacement', 'FontSize', 12);
xlabel('Time (Seconds)')
ylabel('degree', 'FontSize', 10);
...
xlim([0, ff_pitch_ts])
ylim([-20, 20])
xticks(0:ff_pitch_ts/rcrd_time*10:ff_pitch_ts)
xticklabels(0:10:rcrd_time);
yline(0, 'k--');
...
% Create a single legend for the entire figure
lgd = legend('With Ballast', 'Without Ballast', 'FontSize', 10, 'FontWeight', 'bold', 'FontName', 'Helvetica');
lgd.Position = [0.4, 0.005, 0.2, 0.04];
